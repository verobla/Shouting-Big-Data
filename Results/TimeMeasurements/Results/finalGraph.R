...
for (j in 12:26)
{
	...
	assign(nam, 1:j)
	title <- paste("Number of shouting tweets per minute - Jun ",j,sep = "")
	...
	if(length == 4)
	{
		...
		nrow(match)
		graph <- ggplot()+geom_line(aes(x=sub$time,y=sub$count), data = sub, binwidth = 1)+xlab("Time") + ylab("Count")+ggtitle(title)
